...
\title[Running Title]{Non-parametric Bayesian State Estimator for Negative Information} 
...
Simultaneous Localisation and Mapping (SLAM) is concerned with the development of filters to accu-
rately and efficiently infer the state parameters (position, orientation,...) of an agent and aspects of its
environment, commonly referred to as the map. It is necessary for that the agent achieves situatedness
which is a precondition planning and reasoning. In this work we consider an agent who is given the task of
finding a set of objects. The agent has limited perception and can only sense the presences of objects if a
direct contact is made. In the absence of recurrent sightings or direct measurements of objects there are no
correlations from the measurement errors which can be exploited. This renders SLAM estimators, for which
this fact is their backbone, ineffective. No assumptions are taken with respect to the marginals (beliefs) of
both the agent and feature-based. This leads to additional restrictions on current SLAM methodologies for
which the parametrisation of the agent’s and object’s joint distribution is a Multivariate Gaussian. From
the loose assumptions we stipulate regarding the marginals, we adopt a histogram parametrisation.
We introduce a SLAM algorithm, which we name Measurement Likelihood Memory Filter (MLMF), in
which the joint distribution is not parametrised but instead we directly apply changes from the filtering step
to the marginals.
This is achieved by keeping track of the history of measurement likelihood functions. We demonstrate
that our approach gives the same filtered marginals as a histogram filter and show two implementations
in which both have linear space complexity with one showing an exponential time complexity (although
an order of magnitude smaller than the histogram filter) and the other showing a linear time complexity
which we call scalable-MLMF. We further quantitatively demonstrate the scalability of our algorithm with
25 beliefs having up to 10’000’000 states. In an Active-SLAM setting we evaluate the impact that the size
...
\tiny
 \keyFont{ \section{Keywords:} Negative Information, SLAM, Bayesian State Estimator} %All article types: you may provide up to 8 keywords; at least 5 are mandatory.
\end{abstract}
...
\end{center}
 ...
 %Through the memorising of the measurement likelihood functions having been applied on the joint distribution 
%and by taking advantage of their structure, we achieve a filter which grows linearly as opposed to exponentially 
%thisin both time and space complexity.
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{./ch5-MLMF/Figures/Figure2.pdf}
...
\begin{figure}
 \centering
 \includegraphics[width=0.8\textwidth]{./ch5-MLMF/Figures/explenation/hist_SLAM_likelihood.pdf}
...
P(Y_t=1|A_t,O) =
  ...
  \end{cases}
\end{equation}
Figure \ref{fig:histogram_joint} (\textit{Bottom}), illustrates the likelihood of Equation \ref{eq:ch5:discrete_likelihoood} 
...
region which gets affected by the likelihood. When there is no measurement (\textit{Left subfigure}) all the parameters of the 
...
and the values of the joint distribution in those states, $P_{\cap}(A_t,O|Y_{0:t},u_{1:t})$, will be unchanged 
$P_{\ominus}(A_t,O|\mathbf{Y_{0:t}},u_{1:t}) \propto P_{\ominus}(A_t,O|\mathbf{Y_{0:t-1}},u_{1:t})$. 
When the object is detected (\textit{Right subfigure}) the likelihood constrains all non-zero values of the joint 
...
\begin{figure}
 \centering
  \includegraphics[width=0.8\textwidth]{./ch5-MLMF/Figures/explenation/hist_motion.pdf}
...
This is an \textbf{important aspect} which will be present in MLMF.
...
dependent states and updates directly to the marginals without parameterising the values of the joint distribution.
...
$P(A_t|Y_{0:t},u_{1:t})$ and $P(O|Y_{0:t})$ giving a total of $N M$ parameters (where $N$ is the number of states and $M$ is the number of random variables). 
...
of $N^M$ parameters as previously stated in the case of the histogram filter. We propose a method in which we \textbf{do not parameterise the values of the joint
distribution} but rather only compute its impact on the marginals. 
...
\begin{align}
 ...
 &\mathrm{\textbf{measurement}}\nonumber\\
 &P(A_t,O,Y_{0:t}|u_{1:t}) = P(Y_t|A_{t},O)\, P(A_t,O,Y_{0:t-1}|u_{1:t-1})  \label{eq:ch5:mlmf_measurement_update} \\[0.2cm]
 &\mathrm{\textbf{joint}}\nonumber\\
 &P(A_t,O|Y_{0:t},u_{1:t};\mathcolor{red}{\ThOs},\mathcolor{blue}{\ThAs},\mathcolor{dark-green}{\boldsymbol{\Psi_{0:t}}},\mathcolor{dark-blue}{\BAlph}) = \nonumber \\[0.4cm] 
 &\hspace*{4cm}\frac{P(O;\mathcolor{red}{\ThOs})\, P(A_t|u_{1:t};\mathcolor{blue}{\ThAs})\, P(Y_{0:t}|A_t,O,u_{1:t};\mathcolor{dark-green}{\boldsymbol{\Psi_{0:t}}})}{P(Y_{0:t}|u_{1:t};\mathcolor{dark-blue}{\BAlph}) } \label{eq:ch5:mlmf_filter_conditional}  \\ 
 &\mathrm{\textbf{filtered marginal}}\nonumber\\
 &P(A_t|Y_{0:\mathbf{t}};\mathcolor{blue}{\ThA}) = P(A_t|Y_{0:\mathbf{t-1}};\mathcolor{blue}{\ThA}) - \Big(P_{\cap}(A_t|Y_{0:t-1}) -  \mathbf{P_{\cap}(A_t|Y_{0:t}})  \Big)   \label{eq:marignal_mrf}  \\
 &P(O|Y_{0:\mathbf{t}};\mathcolor{red}{\ThO}) = P(O|Y_{0:\mathbf{t-1}};\mathcolor{red}{\ThO}) - \Big(P_{\cap}(A_t|Y_{0:t-1}) -  \mathbf{P_{\cap}(A_t|Y_{0:t}})  \Big)    
 \end{align}
\end{minipage}
};
...
$P(A_t|Y_{0:t},u_{1:t};\mathcolor{blue}{\ThA})$ ($u_{1:t}$ not shown for easing the notation), $P(O|Y_{0:\mathbf{t}};\mathcolor{red}{\ThO})$, 
...
Figure \ref{fig:maringal_joint_example_v2} illustrates the evolution of the non-normalised MLMF joint distribution, Equation  \ref{eq:ch5:mlmf_filter_joint}.
...
\begin{figure}
 \centering
 \includegraphics[width=0.65\textwidth]{./ch5-MLMF/Figures/explenation/example_marginal.pdf}
 \caption{Unnormalised MLMF joint distribution, numerator of Equation \ref{eq:ch5:mlmf_filter_conditional}, at time $t=2$.
 ...
 of multiplying $P(A_2,O|u_{1:2})$ with  $P(Y_{0:2}|A_2,O,u_{1:2})$, giving the numerator of Equation \ref{eq:ch5:mlmf_filter_conditional}. }
...
\begin{equation}
 P(A_t|Y_{0:\mathbf{t}},u_{1:t};\mathcolor{blue}{\ThA}) = P(A_t|Y_{0:\mathbf{t-1}},u_{1:t};\mathcolor{blue}{\ThA}) - \Big(P_{\cap}(A_t|Y_{0:t-1},u_{1:t}) -  \mathbf{P_{\cap}(A_t|Y_{0:t},u_{1:t}})  \Big)   \label{eq:marignal_mrf}  
\end{equation}
\begin{align}
 &\mathbf{P_{\cap}(A_t|Y_{0:t},u_{1:t}};\mathcolor{blue}{\ThAs},\mathcolor{red}{\ThOs},\mathcolor{dark-green}{\BPsi},\mathcolor{dark-blue}{\BAlph})  = \sum\limits_{O} \mathbf{P_{\cap}(A_t,O|Y_{0:t},u_{1:t}};\mathcolor{blue}{\ThAs},\mathcolor{red}{\ThOs},\mathcolor{dark-green}{\BPsi},\mathcolor{dark-blue}{\BAlph}) \nonumber \\[0.5cm]
 &\hspace*{2cm} =\frac{ \sum\limits_{O} P_{\cap}(O;\mathcolor{red}{\ThOs}) P_{\cap}(A_t|u_{1:t};\mathcolor{blue}{\ThAs})  P(Y_{0:t}|A_t,O,u_{1:t};\mathcolor{dark-green}{\BPsi})}{P(Y_{0:t}|u_{1:t};\mathcolor{dark-blue}{\BAlph})} \label{eq:marignal_mrf_2} 
\end{align}

\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{./ch5-MLMF/Figures/explenation/marginal_cal_example_v2.pdf}
...
$P(A_t|Y_{0:t-1},u_{1:t};\mathcolor{blue}{\ThA})$, $P(O|Y_{0:t-1};\mathcolor{red}{\ThO})$ leading to $P(A_t|Y_{0:t};\mathcolor{blue}{\ThA})$, $P(O|Y_{0:t};\mathcolor{red}{\ThO})$ (we did not show $u_{1:t}$ for ease of notation). }
\label{fig:ch5:marginal_update}
\end{figure}

Equation \ref{eq:marignal_mrf} is recursive, $P(A_t,|Y_{0:\mathbf{t}},u_{1:t};\mathcolor{blue}{\ThA})$ is computed in terms of $P(A_t|Y_{0:\mathbf{t-1}},u_{1:t};\mathcolor{blue}{\ThA})$. 
...
the \textbf{filtered marginals} $P(A_t|Y_{0:t},u_{1:t};\mathcolor{blue}{\ThA})$, $P(O|Y_{0:t};\mathcolor{red}{\ThO})$. 
The shape of the \textbf{joint marginals} $P(A_t|u_{1:t};\mathcolor{blue}{\ThAs})$, $P(O;\mathcolor{red}{\ThOs})$ (not shown here, see Figure \ref{fig:maringal_joint_example_v2}) remain unchanged by measurements during the filtering process. 
They are the parameters of the joint distribution used to update the filtered marginals. Table \ref{tab:mlmf_parameters} summarises the functions and parameters of the MLMF for two random variables, 
an agent and object.
...
\cite{PF_tutorial_2002}}. Next we evaluate both space and time complexity of the MLMF filter.
...
%amakes changes to the filter marginals, see Figure \ref{fig:ch5:marginal_update}. 
...
\section{Space \& time complexity}\label{ch5:space_time_complexity_MLMF}
...
\begin{figure}
 \centering
  \includegraphics[width=0.8\textwidth]{./ch5-MLMF/Figures/Figure8_v2.pdf}
...
\begin{figure}
\centering
 \includegraphics[width=0.9\textwidth]{./ch5-MLMF/Figures/Figure12_v2.pdf}
...
\begin{figure}
\centering
 \includegraphics[width=0.8\textwidth]{./ch5-MLMF/Figures/Figure13_v2.pdf}
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{./ch5-MLMF/Figures/exploration_initialisation.pdf}
...
\begin{figure}	
  \centering
  \includegraphics[width=0.5\textwidth]{./ch5-MLMF/Figures/exper_mem_1d.pdf}
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{./ch5-MLMF/Figures/exper_mem_all.pdf}
...
\subsection{Scalabe-MLMF Algorithm}

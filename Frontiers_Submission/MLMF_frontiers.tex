...
Estimating the location of a mobile agent whilst simultaneously building a map of the environment has been
regarded as one of the most important problems to be solved for agents to achieve autonomy. It is a necessary precondition for 
any agent to have an estimation of the world at its disposal which accurately maintains all acquired knowledge. 
There has been a vast amount of research surrounding the field of Simultaneous Localisation And Mapping (SLAM) which branches out into a wide variety of sub-fields 
dealing with problems from building accurate noise models of the agent sensors \cite{Plagemann07gaussianbeam}, to determining 
which environmental feature caused a particular measurement, also known as the data association problem \cite{DataAssociation2003} 
and many more. 

% Why does SLAM Work

Although the amount of research might seem overwhelming at first, all current SLAM methodologies are founded on a single principle; 
the uncertainty of the map's features are correlated through successive measurements of agent obtained at different locations \cite{SLAM_part_I_2006}. 
As the agent localises itself by reducing its position uncertainty, the maps' uncertainty also decreases as the landmarks are 
correlated. The predominant mathematical formulation of a SLAM problems is to parameterise the joint distribution of the agent's position and maps' features and to recursively 
integrate applied actions and senses measurements, which is referred as Probabilistic-SLAM \cite{Cadena16tro-SLAMfuture,SLAM_part_I_2006}.


%mathematical approach is to recursively filter the joint distribution of the agent and map locations through the successive application 
%of a motion and measurement model, also known as the system models.
% The three main pillar of SLAM algorithm and their respective draw backs

There are broadley three main paradigms to parameterising the joint distribution and integrating the sensed information:

The first paradigms assumes the joint distribution to be a Multivariate Gaussian distribution and is parameterised by a mean and covariance. The 
mean is a state vector composed of the agent and map feature positions whilst the elements of the covariance encode the position uncertainty 
of the state vector. If the motion and measurement system models are linear the Kalman Filter recursion is the best estimator 
of the joint's parameters. In case the system models are non-linear they can be linearised via a Taylor approximation and in the SLAM context this 
is known as EKF-SLAM (Extendend-Kalman Filter) \cite{SLAM_part_I_2006} an early popular method.

% The problem with EKF-SLAM : works only with 100 of states has a computational cost which is quadratic [cite]

% UKF : sample based approximation, sigam points, No Jacobian needed during Taylor approximation
Unscented Kalman Filter\cite{wan2000} \cite{UKF_SLAM_2013}, 

% EIF 
% Paramerises are the canonical ones of the Gaussian, 
Extended information Filter \cite{EIF_2004}, \cite{SEIF_2014}.



\cite{VAIN_2014} reviewed the inconsistencies in vision-aided navigation system (VINS) in which data from camera and 
inertial measurement units are fused to estimate the position and orientation of a sensor. Main source of inconsistency
is due to the linearisation of the measurement function which falsely reduces the uncertainty in pose estimation which 
results an increase of the estimation error. The authors introduce a constraint on the Jacobian of both motion and measurement 
functions which explicitly enforces that no unwanted information is gained. The authors demonstrated their EKF-SLAM with additional 
constraints on 1.5km outdoor navigation and mapping of the University of Minnesota campus.

% Need to stay that 


\cite{High_precision_EKF_2013} multi-state-constraint Kalman filter MSCK F2.0 is a real-time 
EKF-SLAM for visual-inertial odometry (VIO) sensory information. Linearisation of system models 
(motion and measurement models) lead to the uncertainty in the state being underestimated. 

MSCK \cite{MSCKF_2007} algorithm outperforms EKF-SLAM methods by a wide margin. As oppose to EKF-SLAM MSCKF maintains 
a sliding window of poses in its state vector and uses feature measurements to impose constraints on these 
poses. MSCKF is linear in the number of features. Performs also online camera calibration.

% a review of underwater slam techniques

\cite{Review_SLAM_2015}

FastSLAM: one drawback is that information is lost by during resampling process.
...
% complexiy is quadradit with the number of features [citation]
% cannot correct erroneous data associations
% the point of linearization should be adequate.


% Need to talk about the latest state of the art for EKF-SLAM.
...
% no issue with data associate (solves the full slam problem (full trajectory of the robot) as oppose to the online slam problem.

% represent the probability distribution by a set of particules

% what is the state of the art in SLAM ?
...
the sensor and motion models. The reason for this wide spread limitation is that during theoretical foundation of SLAM, 
a period which is referred to as the \textit{classic age} (1986-2004)\cite{Cadena16tro-SLAMfuture} (this period gave the three paradigms
discussed above), the application problems considered the signal noise in range sensor, cameras and odometry measurements.
The features were always considered to be measurable except when occluded. In our setting as the sensory information is strictly haptic, 
...
of EKF-SLAM and Graph-SLAM, will not perform well in this setting as no measurements of the features positions available until our agent ``bumps''
into a feature.  
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{./Figures/Figure13.pdf}
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{./Figures/Figure15.pdf}
